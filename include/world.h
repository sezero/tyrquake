...
SV_TraceMoveEntity(const edict_t *entity, const vec3_t start, const vec3_t end, movetype_t type, trace_t *trace)
{
    return SV_TraceMove(start, entity->v.mins, entity->v.maxs, end, type, entity, trace);
}

static inline const edict_t *
SV_TraceLine(const vec3_t start, const vec3_t end, movetype_t type, const edict_t *passedict, trace_t *trace)
{
