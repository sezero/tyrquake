...
R_InitSky(texture_t *mt)
{
    ...
    pic.pixels = src + 128;
    mt->gl_texturenum = GL_LoadTexture("@sky_background", &pic, TEXTURE_TYPE_SKY_BACKGROUND);

    /* Create the alpha layer */
    pic.pixels = src;
    mt->gl_texturenum_alpha = GL_LoadTexture("@sky_foreground", &pic, TEXTURE_TYPE_SKY_FOREGROUND);
}
